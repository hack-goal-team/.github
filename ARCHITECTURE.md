...
**Инференс** живёт отдельным контейнером рядом с приложением и ходит в Postgres под своей ролью `inference` (ADR-025). Единица прогноза — объект, разрез по инженерным системам идёт через тип инцидента (ADR-024). До обученной модели в контейнере работает мок, пишущий вырожденные строки с `model_version='mock-<N>'` (HACK-31).
